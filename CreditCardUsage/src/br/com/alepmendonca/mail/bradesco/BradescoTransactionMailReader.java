...
public class BradescoTransactionMailReader {
	...
	public CardReceipt extractMessage(Multipart message, InputStream pdf) throws IOException, ParseException, MessagingException {
		if (canExtract(message)) {
			...
			CardReceipt receipt = createCardReceipt(pdfStrategy.getResultantText(), strategy.context);
	...
	private CardReceipt createCardReceipt(String receipt, Context c) throws ParseException {
		...
		CreditCardUsageApplication context = (CreditCardUsageApplication) c.getApplicationContext();
		final DaoSession daoSession = context.getDAO();
